...
class WCSPHSolver(SPHBase):
    def __init__(self, particle_system):
        ...
        self.d_velocity = ti.Vector.field(self.ps.dim, dtype=ti.f32)
    ...
    def compute_non_pressure_forces(self):
        for p_i in range(self.ps.particle_num[None]):
            ...
            if self.ps.material[p_i] == self.ps.material_fluid:
                ...
                # Other up force 
                w = ti.static(self.up_area)
                if self.ps.x[p_i].x > w[0]+0.1 and self.ps.x[p_i].x < w[1]-0.1 and self.ps.x[p_i].y > 0 and self.ps.x[p_i].y < 4.5 :
                    d_v += ti.Vector([0.0, -2*self.g] if self.ps.dim == 2 else [0.0, 0.0, -self.g])
            self.d_velocity[p_i] = d_v
    ...
    def advect(self):
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            if self.ps.material[p_i] == self.ps.material_fluid:
                vi              = self.ps.v[p_i] + self.dt[None] * self.d_velocity[p_i]
                xi              = self.ps.x[p_i] + self.dt[None] * vi
