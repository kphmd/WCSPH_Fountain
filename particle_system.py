...
class ParticleSystem:
    def __init__(self, res):
        ...
        self.x0                     = ti.Vector.field(self.dim, dtype=ti.f32)
        ...
        self.particles_node.place(self.x0, self.x, self.v, self.density, self.pressure, self.material, self.color)
        ...
        self.particle_node          = self.particles_node.pointer(ti.j, self.particle_max_num_neighbor//10).dense(ti.j, 10)
        ...
        self.grid_node              = ti.root.pointer(index, (self.grid_num + 15) // 16).dense(index, (16,16) )
        ...
        cell_node                   = self.grid_node.pointer(cell_index, self.particle_max_num_per_cell//10).dense(cell_index,10)
        cell_node.place(self.grid_particles)

        # wall line
        self.lines             = ti.Vector.field(2,ti.f32,shape=(12*2,))

        self.lines[ 0*2+0]      = [4-1/2,7]
        self.lines[ 0*2+1]      = [4-2/2,2]
        self.lines[ 1*2+0]      = [4+1/2,7]
        self.lines[ 1*2+1]      = [4+2/2,2]
        self.lines[ 2*2+0]      = [4-1/3,7.5]
        self.lines[ 2*2+1]      = [4    ,7]
        self.lines[ 3*2+0]      = [4+1/3,7.5]
        self.lines[ 3*2+1]      = [4    ,7]
        self.lines[ 4*2+0]      = [4    ,7]
        self.lines[ 4*2+1]      = [4    ,5]
        self.lines[ 5*2+0]      = [4+2/3,7.5]
        self.lines[ 5*2+1]      = [4-2/3,7.5]

        self.lines[ 6*2+0]      = [                self.padding,                self.padding]
        self.lines[ 6*2+1]      = [                self.padding,self.bound[1] - self.padding]
        self.lines[ 7*2+0]      = [                self.padding,self.bound[1] - self.padding]
        self.lines[ 7*2+1]      = [self.bound[0] - self.padding,self.bound[1] - self.padding]
        self.lines[ 8*2+0]      = [self.bound[0] - self.padding,self.bound[1] - self.padding]
        self.lines[ 8*2+1]      = [self.bound[0] - self.padding,                self.padding]
        self.lines[ 9*2+0]      = [self.bound[0] - self.padding,                self.padding]
        self.lines[ 9*2+1]      = [                self.padding,                self.padding]

        self.lines[10*2+0]      = [7-1/2, 7    ]
        self.lines[10*2+1]      = [7+1/2, 7    ]
        self.lines[11*2+0]      = [7    , 7-1/2]
        self.lines[11*2+1]      = [7    , 7+1/2]

    @ti.kernel 
    def mv_cross(self, p_x : ti.f32, p_y : ti.f32) :
        p_x                  = ti.min(ti.max(p_x,2),8)
        p_y                  = ti.min(ti.max(p_y,2),8)

        p0                   = ti.Vector([self.lines[11*2+0][0], self.lines[10*2+0][1]])
        p1                   = ti.Vector([                  p_x,                   p_y])

        p1                   = p1-p0

        if p1.norm() > self.particle_radius * 0.5 :
            p1 = p1.normalized()
            self.lines[10*2+0]  += p1 * self.particle_radius * 0.5
            self.lines[10*2+1]  += p1 * self.particle_radius * 0.5
            self.lines[11*2+0]  += p1 * self.particle_radius * 0.5
            self.lines[11*2+1]  += p1 * self.particle_radius * 0.5
    ...
    def add_particles(self, new_particles_num: int,
        ...
        for p in range(self.particle_num[None], self.particle_num[None] + new_particles_num):
            p_new = p - self.particle_num[None]

            v     = ti.Vector.zero(ti.f32, self.dim)
            x     = ti.Vector.zero(ti.f32, self.dim)

            for d in ti.static(range(self.dim)):
                v[d] = new_particles_velocity [p_new, d]
                x[d] = new_particles_positions[p_new, d]

            self.add_particle(p, x, v,
                              new_particle_density  [p_new],
                              new_particle_pressure [p_new],
                              new_particles_material[p_new],
                              new_particles_color   [p_new])
    ...
    def add_cube(self,
        ...
        new_positions = new_positions.reshape(
            -1,
            reduce(lambda x, y: x * y, list(new_positions.shape[1:]))
        ).transpose()

        print("new position shape ", new_positions.shape)
        if velocity is None:
            velocity = np.full_like(new_positions, 0, dtype=np.float32)
        else:
            velocity = np.array([velocity for _ in range(num_new_particles)], dtype=np.float32)

        material = np.full_like(np.zeros(num_new_particles), material, dtype=np.float32)
        color    = np.full_like(np.zeros(num_new_particles), color, dtype=np.float32)
        density  = np.full_like(np.zeros(num_new_particles), density  if density  is not None else 1000., dtype=np.float32)
        pressure = np.full_like(np.zeros(num_new_particles), pressure if pressure is not None else 0., dtype=np.float32)
